...
<div class="screen screen__front">
    ...
    <form id="form" class="form" method="POST" action="{{ route('login') }}">
        ...
        @csrf
        <span class="form__field form__field--email @error('password') form__field--error @enderror"><input id="form__field--email" type="email" name="email" value="{{ old('email') }}" placeholder="Email" minlength="6" max="255" required autofocus></span>
        ...
        @if (Route::has('register'))<a href="{{ route('register') }}" class="link">or Register</a>@endif
